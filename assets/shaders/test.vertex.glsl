...
void main() {
	v_diffuseUV = u_diffuseUVTransform.xy + a_texCoord0;
	
	#ifdef emissiveTextureFlag
		v_emissiveUV = u_emissiveUVTransform.xy + a_texCoord0;
	#endif //emissiveTextureFlag

	#ifdef specularTextureFlag
		v_specularUV = u_specularUVTransform.xy + a_texCoord0;
	...
	gl_Position = snap(u_projViewTrans * pos, vec2(120, 90));
		
	v_uv = vec3(v_diffuseUV * gl_Position.w, gl_Position.w);
